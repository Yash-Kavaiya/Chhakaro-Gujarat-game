...
describe('passportProgress', () => {
  it('is 0/17 · 0% for no visits', () => {
    expect(passportProgress(GUJARAT_LOCATIONS, [])).toEqual({ visited: 0, total: 17, pct: 0 });
  });
  it('rounds pct', () => {
    const three = GUJARAT_LOCATIONS.slice(0, 3).map((l) => l.id);
    expect(passportProgress(GUJARAT_LOCATIONS, three)).toEqual({ visited: 3, total: 17, pct: 18 });
  });
